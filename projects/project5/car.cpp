...
Car::Car(const Car &aCar)
{
	...
	mpg = aCar.mpg;
        ...
        displacement = aCar.displacement;
        horsepower = aCar.horsepower;
        weight = aCar.weight;
        acceleration = aCar.acceleration;
        model = aCar.model;
        origin = aCar.origin;
}
...
void Car::printCar(ofstream &outFile)
{
        char originDesc[MAXCHAR];
        printOrigin(this->origin, originDesc);
        outFile << carName << ';' << mpg << ';' << cylinders << ';' << displacement << ';' << horsepower << ';' << weight << ';' << acceleration << ';' << model << ';' << originDesc << endl;
}
...
Car& Car::operator= (const Car& aCar)
{
        ...
        {
                carName = new char[strlen(aCar.carName) + 1];
		strncpy(this->carName, aCar.carName, strlen(aCar.carName) +1);
                this->mpg = aCar.mpg;
